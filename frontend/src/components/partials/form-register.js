				...
				{vm.state.success ?
					<React.Fragment>
						...
						<div className="loginform-nav center">
							<button type="button" className="nav-btn" onClick={closeModal}>Close</button>
						</div>
					</React.Fragment>
					:
					<React.Fragment>
						<h2 className="loginform-title">Create a New Account</h2>
						<InputForm className="loginform-form" onSubmit={this.submit} onTouch={() => { vm.setState({ touched: true }) }}>
							...
							<FormInput
								...
								type="email"
								label="E-Mail"
								validation={{ required: "Please enter a valid e-mail.", email: true }}
								className="form-field" />
							<FormInput
								name="password"
								...
								type="password"
								label="Password"
								validation={{ required: "Please enter your password", minLength: ["Your password must have at least {length} characters.", 6], "compare": ["Passwords are not matching.", "#register-password-repeat"] }}
								className="form-field" />
							<FormInput
								...
								validation={{ required: "Please enter your password", minLength: ["Your password must have at least {length} characters.", 6], "compare": ["Passwords are not matching.", "#register-password"] }}
								className="form-field" />
							<Btn
								className="form-field"
								type="submit"
								...
								block uppercase light>Sign up</Btn>
						</InputForm>
						<div className="loginform-nav">
							<span>
								Already have an account? <button type="button" className="nav-btn" onClick={() => { vm.goToLogin() }}>Sign in</button>
							</span>
						</div>
					</React.Fragment>
				}
