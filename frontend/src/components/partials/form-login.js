	...
	render() {
		...
		return (
			<div className={"section loginform type-" + vm.props.type}>
				...
				<InputForm className="loginform-form" onSubmit={this.submit} autoComplete="off">
					{vm.state.message &&
					...
					<FormInput
						...
						label="Password"
						disabled={vm.state.loading}
						validation={{ required: "Please enter your password", minLength: ["Your password must have at least {length} characters.", 6] }}
						className="form-field" />
					<Btn
						className="form-field"
						type="submit"
						...
						block uppercase light>Login</Btn>
				</InputForm>
				<div className="loginform-nav">
					<span>Not registered yet? <button type="button" className="nav-btn" onClick={() => { vm.goToRegister() }}>Sign up now</button></span>
				</div>
			</div>
		)
	}
